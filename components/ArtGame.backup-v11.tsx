            ...
            {(['restoration', 'puzzle', 'quiz'] as const).map((m, i) => {
              ...
              return (
                <button key={m} onClick={() => handleModeSelect(m)}
                  ...
                  <div className="absolute inset-0 bg-gradient-to-t from-black/80 via-black/20 to-black/5 group-hover:via-black/40 transition-all duration-700" />
                  ...
                  <div className="absolute inset-x-0 bottom-0 p-4 md:p-7 z-20">
                    <h3 className="font-serif text-2xl md:text-4xl text-white mb-1 md:mb-2 drop-shadow-lg">{t(`game.mode_${m}`)}</h3>
                    <p className="text-white/60 text-xs md:text-sm leading-relaxed max-w-[240px]">
                      {t(`game.desc_${m}`)}
                    </p>
                    <div className="flex items-center gap-2 mt-3 md:mt-4">
                      <span className="text-[9px] font-bold uppercase tracking-[0.4em] text-white/70">{t('game.start')}</span>
