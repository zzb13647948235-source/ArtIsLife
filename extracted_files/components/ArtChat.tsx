...
import { Send, Bot, User, Quote, ArrowRight, Sparkles, ExternalLink, Search, Bookmark, Lock, LogIn } from 'lucide-react';
...
const ArtChat: React.FC<ArtChatProps> = ({ messages, setMessages, onAuthRequired, isLoggedIn }) => {
  ...
  const handleSend = async (text: string) => {
    ...
    try {
      ...
      const systemInstruction = t('chat.system_instruction');
      const response = await chatWithArtExpert(text, history, systemInstruction);
  ...
  return (
    <div className="pt-28 pb-12 px-6 h-screen max-w-[1800px] mx-auto flex flex-col md:flex-row gap-12 lg:gap-24 animate-fade-in">
      ...
      <div className="hidden md:flex flex-col w-80 pt-12 sticky top-28 h-fit space-y-10">
        <div className="space-y-2">
            <h2 className="font-serif text-6xl text-art-accent leading-tight italic">
                <AppleText text="Art" delay={0.1} /> <br/>
                <AppleText text="Curator" delay={0.3} />
            </h2>
        ...
        <div className="bg-white p-10 rounded-[32px] shadow-hard border border-stone-50 relative overflow-hidden group">
            ...
            <p className="text-stone-700 text-base leading-relaxed font-serif italic relative z-10">
            "{t('chat.quote')}"
            </p>
        </div>
        ...
        {!isLoggedIn && (
            <div className="p-8 bg-art-accent text-white rounded-[32px] shadow-xl relative overflow-hidden">
                ...
                <h4 className="text-xs font-bold uppercase tracking-widest mb-4 flex items-center gap-2">
                    <LogIn size={14} className="text-art-primary" /> 加入深度对话
                </h4>
                <p className="text-xs text-white/60 leading-relaxed mb-6">
                    只有注册会员才能直接向策展人提问，获取大师级艺术见解。
                </p>
      ...
      <div className="flex-1 bg-white shadow-hard rounded-[48px] flex flex-col overflow-hidden relative border border-stone-50 transition-all duration-700 hover:shadow-2xl">
        <div 
          ref={scrollRef}
          className="flex-1 overflow-y-auto p-10 md:p-16 space-y-12 bg-[#FBFAF9] scrollbar-thin scrollbar-thumb-stone-200"
        >
          {messages.map((msg) => (
            <div key={msg.id} className={`flex gap-6 animate-fade-in-up ${msg.role === 'user' ? 'flex-row-reverse' : ''}`}>
              ...
              <div className={`flex flex-col max-w-[85%] ${msg.role === 'user' ? 'items-end' : 'items-start'}`}>
                  <div className={`
                    relative rounded-[32px] p-8 shadow-medium text-lg font-serif leading-[1.8] tracking-tight
                    ${msg.role === 'user' 
                        ? 'bg-stone-800 text-stone-100 rounded-tr-none' 
                        : 'bg-white border border-stone-50 text-stone-800 rounded-tl-none'
                    }
                  `}>
                     <ReactMarkdown className="prose prose-stone prose-lg max-w-none prose-p:italic">{msg.text}</ReactMarkdown>
                  </div>
                              ...
                              {msg.groundingLinks.map((link, lIdx) => (
                                  ...
                                  >
                                      <div className="flex justify-between items-start">
                                          ...
                                          <span className="text-[10px] font-bold uppercase tracking-widest text-stone-300 group-hover:text-art-primary/40">
                                              {link.source || 'Web'}
                                          </span>
                                      </div>
                                      <h5 className="font-serif text-sm text-stone-800 line-clamp-1 leading-snug group-hover:text-art-primary transition-colors">
                                          {link.title}
                                      </h5>
                                  </a>
                              ))}
        ...
        <div className="p-10 md:p-14 bg-white border-t border-stone-50 relative group">
           <div className="relative group/input">
              {!isLoggedIn && (
                  <div className="absolute inset-0 bg-white/5 backdrop-blur-[1px] z-20 cursor-pointer rounded-full" onClick={onAuthRequired}></div>
              )}
              {/* FIXED: Input z-index increased to be interactive when logged in */}
              ...
              <input
                ...
                placeholder={isLoggedIn ? t('chat.placeholder') : '签名进入沙龙即可发起对话...'}
                className="w-full pl-10 pr-20 py-8 bg-stone-50 border border-stone-100 rounded-full focus:bg-white focus:ring-1 focus:ring-art-primary/20 focus:border-art-primary/30 outline-none font-serif text-xl shadow-inner transition-all italic relative z-10"
                disabled={loading}
              />
              <button
                onClick={() => handleSend(input)}
                disabled={loading || (isLoggedIn && !input.trim())}
                className={`
                    absolute right-3 top-3 p-6 rounded-full transition-all active:scale-90 shadow-2xl z-30
                    ${loading || (isLoggedIn && !input.trim()) 
                        ? 'bg-stone-200 text-stone-400' 
                        : 'bg-art-accent text-white hover:bg-art-primary hover:scale-105'
                    }
                `}
              >
